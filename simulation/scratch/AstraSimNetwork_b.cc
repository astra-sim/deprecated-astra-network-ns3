...
int recv_counter=0;
int A2A_step=-1;
std::vector<AstraSim::AstraNetworkAPI*> networks(4,nullptr);
...
void unoptimized_A2A_Step(void *arg){
    recv_counter++;
    if(recv_counter==num_gpus-1){
        A2A_step++;
        recv_counter=0;
        if(A2A_step==num_gpus){
            return;
        }
    } else{
        return;
    ...
    for(int i=0;i<num_gpus;i++){
        if(i==A2A_step){
            ...
            networks[i]->sim_send(nullptr, //not yet used
                                  1024*1024, //number of bytes to be send
                                  0,//not yet used
                                  A2A_step,
                                  0, //not yet used
...
int main (int argc, char *argv[]){
    ...
    A2A_step=-1;
    recv_counter=num_gpus-2;
    unoptimized_A2A_Step(nullptr);
