...
int job_num = 1;
// std::vector<std::string> ml_models = {"../astra-sim/inputs/workload/DLRM_HybridParallel.txt", "../astra-sim/inputs/workload/MLP_ModelParallel.txt", "../astra-sim/inputs/workload/Transformer_HybridParallel.txt"};
std::vector<std::string> ml_models; //= {   "../astra-sim/inputs/workload/microAllToAll.txt", "../astra-sim/inputs/workload/microAllReduce.txt",  };
...
public:
  ...
  int job_id;
  int network_node_id;
  ASTRASimNetwork(int rank, int _job_id, int _network_node_id) : AstraNetworkAPI(rank) {
  ...
  virtual void sim_schedule(AstraSim::timespec_t delta,
    ...
    std::cout << " ns3 sim schedule is called at " << Simulator::Now().GetTimeStep() << endl;
    return;
  }
  virtual int sim_send(void *buffer,   // not yet used
    ...
    task1 t;
    t.src = network_node_id;//job_nodes_sets[job_id][rank]; // how to get src, is it the rank (starts from 0?)
    t.dest = rank_to_network_node_map[dst]; //dst; //job_nodes_sets[job_id][dst];
    
    std::cout<< Simulator::Now().GetTimeStep() <<" sim_send  job_id "<< job_id <<" rank " <<rank  << " to  dst "<< dst
          <<", network src "<< network_node_id << " network dest "<<t.dest << " tag "<<tag <<  " size " << count << std::endl;
    // std::cout<<"sim_send  job_id "<< job_id <<" rank " <<rank  << " network_node_id "<< network_node_id <<" tag "<<tag <<std::endl;

    // std::cout<<"sim_send  job_id "<< job_id <<" src " <<t.src <<" dst "<<t.dest <<" tag "<<tag;
    // std::cout << " size " << count <<"\n";
    // int a = 1;
    t.count = count;
    // t.fun_arg = &a;
  ...
  virtual int sim_recv(void *buffer, uint64_t count, int type, int src, int tag,
    ...
    // populate task1 with the required arguments
    task1 t;
    t.src = rank_to_network_node_map[src];//src;//job_nodes_sets[job_id][src];  //src; 
    t.dest = network_node_id; //job_nodes_sets[job_id][rank]; //rank; // how to get dest, is it the rank (starts from 0?)
    std::cout<< Simulator::Now().GetTimeStep() <<" sim_recv  job_id "<< job_id <<" src " <<src  << " to  dst "<< rank
          <<", network src "<< t.src << " network dest "<<t.dest << " tag "<<tag << std::endl;
    // std::cout<<"sim_recv  job_id "<< job_id <<" src " <<src<<" rank "<<rank<<" tag "<<tag<<"\n";
    // std::cout<<"sim_recv  job_id "<< job_id <<" src " <<t.src<<" dst "<<t.dest<<" tag "<<tag<<"\n";
    // int a = 1;
    t.count = count;
    // std::cout<<"sim_recv  count "<< t.count <<t.src<<" "<<t.dest<<" "<<tag<<"\n";
    // t.fun_arg = &a;
    t.type = 1;
    t.fun_arg = fun_arg;
    t.msg_handler = msg_handler;
    ...
    if (recvHash.find(make_pair(tag, make_pair(t.src, t.dest))) !=
      ...
      if (count == t.count) {
        ...
        std::cout<<"already in recv hash "<<t.src<<" "<<t.dest<<" "<<tag<<"\n";
        t.msg_handler(t.fun_arg);
      } else if (count > t.count) {
        recvHash[make_pair(tag, make_pair(t.src, t.dest))] = count - t.count;
        std::cout<<"already in recv hash with more data "<<t.src<<" "<<t.dest<<" "<<tag<<"\n";
        t.msg_handler(t.fun_arg);
      } else {
        recvHash.erase(make_pair(tag, make_pair(t.src, t.dest)));
        t.count -= count;
      ...
      if (expeRecvHash.find(make_pair(tag, make_pair(t.src, t.dest))) ==
          expeRecvHash.end()) {
        expeRecvHash[make_pair(tag, make_pair(t.src, t.dest))] = t;
        std::cout<<"not in recv hash "<<t.src<<" "<<t.dest<<"  "<<tag<<"\n";
      } else {
        int expecount =
            expeRecvHash[make_pair(tag, make_pair(t.src, t.dest))].count;
        t.count += expecount;
        expeRecvHash[make_pair(tag, make_pair(t.src, t.dest))] = t;
        std::cout<<"not in recv hash but in expected recv hash "<<t.src<<" "<<t.dest<<" "<<tag<<"\n";
      }
      // expeRecvHash[make_pair(tag,make_pair(t.src,t.dest))] = t;
    }
...
int main(int argc, char *argv[]) {
  main1(argc, argv);
  std::cout << "job config: " << job_config << " sys " << sys_config << std::endl;
  ...
  for(int j=0; j<nodes_per_job.size(); j++){
    for(int i=0; i<num_gpus; i++){
      ...
      int x = j*num_gpus + i;
      int network_node_id =  nodes_per_job[j][i];//(int)(i/64) + 16*(i%64); //i*16 + j;
  ...
  for (int j = 0; j < job_num; j++) {
    ...
    for (int i = 0; i < num_gpus; i++) {
      ...
      systems[x] = new AstraSim::Sys(
          ...
          sys_config, //"../astra-sim/inputs/system/"+sys_config, // + system_input, // system configuration
          ml_models[model_id], //microAllReduce.txt DLRM_HybridParallel.txt, microAllToAll.txt, Resnet50_DataParallel.txt,
          (int)physical_dims[0]*communication_scale_factor, // communication scale
          1, // computation scale
          1, // injection scale
          1,
