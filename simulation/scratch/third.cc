...
Ipv4Address node_id_to_ip(uint32_t id) {
  return Ipv4Address(
      0x0b000001 + ((id / 256) * 0x00010000) + ((id % 256) * 0x00000100));
}
...
void monitor_buffer(FILE* qlen_output, NodeContainer* n) {
      ...
      for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
        ...
        if (size >= 1000) {
          ...
          if (test == 0) {
            test = 1;
            fprintf(
                qlen_output, "time %lu %u ", Simulator::Now().GetTimeStep(), i);
          }
          // if(j==1){
  ...
  fflush(qlen_output);
  Simulator::Schedule(
      NanoSeconds(qlen_mon_interval), &monitor_buffer, qlen_output, n);
}
    ...
    for (auto j = table.begin(); j != table.end(); j++) {
      ...
      for (int k = 0; k < (int)nexts.size(); k++) {
        ...
        if (node->GetNodeType() == 1)
          ...
          node->GetObject<RdmaDriver>()->m_rdma->AddTableEntry(
              dstAddr, interface);
        }
      }
    }
...
int main1(int argc, char* argv[]) {
  ...
  Config::SetDefault(
      "ns3::QbbNetDevice::DynamicThreshold", BooleanValue(dynamicth));
  ...
  for (uint32_t i = 0; i < link_num; i++) {
    ...
    DynamicCast<QbbNetDevice>(d.Get(0))->TraceConnectWithoutContext(
        "QbbPfc",
        MakeBoundCallback(
            &get_pfc, pfc_file, DynamicCast<QbbNetDevice>(d.Get(0))));
    DynamicCast<QbbNetDevice>(d.Get(1))->TraceConnectWithoutContext(
        "QbbPfc",
        MakeBoundCallback(
            &get_pfc, pfc_file, DynamicCast<QbbNetDevice>(d.Get(1))));
  }
  ...
  for (uint32_t i = 0; i < node_num; i++) {
    if (n.Get(i)->GetNodeType() == 1) { // is switch
      ...
      for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
        ...
        NS_ASSERT_MSG(
            rate2kmin.find(rate) != rate2kmin.end(),
        ...
        sw->m_mmu->ConfigEcn(
            j, rate2kmin[rate], rate2kmax[rate], rate2pmax[rate]);
        // set pfc
  ...
  for (uint32_t i = 0; i < node_num; i++) {
    if (n.Get(i)->GetNodeType() == 0) { // is server
      ...
      rdmaHw->SetAttribute(
          "AlphaResumInterval", DoubleValue(alpha_resume_interval));
      rdmaHw->SetAttribute("RPTimer", DoubleValue(rp_timer));
      rdmaHw->SetAttribute(
          "FastRecoveryTimes", UintegerValue(fast_recovery_times));
      ...
      rdmaHw->SetAttribute(
          "RateDecreaseInterval", DoubleValue(rate_decrease_interval));
      ...
      rdmaHw->SetAttribute(
          "DctcpRateAI", DataRateValue(DataRate(dctcp_rate_ai)));
  ...
  if (link_down_time > 0) {
    Simulator::Schedule(
        Seconds(2) + MicroSeconds(link_down_time),
        &TakeDownLink,
        n,
        n.Get(link_down_A),
        n.Get(link_down_B));
