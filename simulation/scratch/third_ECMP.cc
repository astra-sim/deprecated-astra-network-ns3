...
void CalculateRoutes(NodeContainer &n){
	...
	int cntg = 0;
	for(int m = 0;m<16;m++){
		//std::cout<<"cnt is "<<cntg<<"\n";
		for(int i = cntg;i<cntg+8;i++)
		{
			for(int j = cntg;j<cntg+8;j++){
			if(i==j){
                        continue;
                        }
                        for(int k = 0;k< (int)nextHop[n.Get(i)][n.Get(j)].size();k++){
                                if(nextHop[n.Get(i)][n.Get(j)][k]->GetId() >= 224){
			//	std::cout<<"erased ij "<<i<<" "<<j<<" "<<nextHop[n.Get(i)][n.Get(j)][k]->GetId()<<"\n";
                                nextHop[n.Get(i)][n.Get(j)].erase(nextHop[n.Get(i)][n.Get(j)].begin()+k);
                                }
                        }
			}
		}
