...
namespace ns3 {
		...
		QbbNetDevice::DequeueAndTransmit(void)
	{
		...
		{	
			std::cout<<Simulator::Now()<<" channel busy \n";
			return;	// Quit if channel busy
			}
			Ptr<Packet> p;
		if (m_node->GetNodeType() == 0){
			int qIndex = m_rdmaEQ->GetNextQindex(m_paused);
			std::cout<<Simulator::Now()<<" qIndex is "<<qIndex<<"\n";
			if (qIndex != -1024){
				std::cout<<Simulator::Now()<<" there are packet to send\n";
				if (qIndex == -1){ // high prio
					std::cout<<Simulator::Now()<<" there are packet to send in high priority\n";
				...
				std::cout<<Simulator::Now()<<" transmit started \n";
				TransmitStart(p);
				...
				std::cout<<Simulator::Now()<<" there are no packet to send\n";
			...
			//	std::cout<<"maximum simulation time is "<<t<<"\n";
				for (uint32_t i = 0; i < m_rdmaEQ->GetFlowCount(); i++){
					...
					if (qp->GetBytesLeft() == 0){
						std::cout<<Simulator::Now()<<" get byte left in dequeue transmit\n";
						qp->m_notifyAppSent();
						continue;
					}
			...
			std::cout<<Simulator::Now()<<" in switch m_paused is "<<m_paused<<"\n";
			p = m_queue->DequeueRR(m_paused);		//this is round-robin
			if (p != 0){
				std::cout<<Simulator::Now()<<" queue can deliver packet \n";
				m_snifferTrace(p);
				...
				std::cout<<Simulator::Now()<<" queue index in switch is "<<qIndex<<"\n";
				if (qIndex == 0){//this is a pause or cnp, send it immediately!
					std::cout<<Simulator::Now()<<" pause or cnp in switch, send immediately\n";
					m_node->SwitchNotifyDequeue(m_ifIndex, qIndex, p);
					p->RemovePacketTag(t);
				}else{
					std::cout<<Simulator::Now()<<" send packet normal from SWITCH\n";
					m_node->SwitchNotifyDequeue(m_ifIndex, qIndex, p);
					p->RemovePacketTag(t);
				}
				m_traceDequeue(p, qIndex);
				std::cout<<Simulator::Now()<<" transmit start in switch \n";
				TransmitStart(p);
				std::cout<<Simulator::Now()<<" transmit done\n";
				return;
			}else{ //No queue can deliver any packet
				std::cout<<Simulator::Now()<<" no queue can delier packet pause prohibits send at node in switch "<<m_node->GetId()<<"\n";
				NS_LOG_INFO("PAUSE prohibits send at node " << m_node->GetId());
				if (m_node->GetNodeType() == 0 && m_qcnEnabled){
					std::cout<<Simulator::Now()<<"nothing to send, possibly due to qcn flow control, if so reschedule sending\n";
					Time t = Simulator::GetMaximumSimulationTime();
					for (uint32_t i = 0; i < m_rdmaEQ->GetFlowCount(); i++){
						Ptr<RdmaQueuePair> qp = m_rdmaEQ->GetQp(i);
						if (qp->GetBytesLeft() == 0){
							std::cout<<Simulator::Now()<<" get byte left in dequeue transmit in switch\n";
							qp->m_notifyAppSent();
							continue;
						}	
						t = Min(qp->m_nextAvail, t);
					...
					if (m_nextSend.IsExpired() && t < Simulator::GetMaximumSimulationTime() && t > Simulator::Now()){
						std::cout<<Simulator::Now()<<" in m_nextSend \n";
						std::cout<<t<<" dequeue and transmit called in switch future call\n";
						m_nextSend = Simulator::Schedule(t - Simulator::Now(), &QbbNetDevice::DequeueAndTransmit, this);
					}
				}
			}
		}
		std::cout<<Simulator::Now()<<" returning from dequeue and transmit \n";
		return;
	}
		...
		QbbNetDevice::Receive(Ptr<Packet> packet)
	{
		...
		std::cout<<Simulator::Now()<<" receive packet \n";
		if (!m_linkUp){
		...
		if (ch.l3Prot == 0xFE){ // PFC
			std::cout<<Simulator::Now()<<" receive pfc packet \n";
			...
			std::cout<<Simulator::Now()<<" receive normal packet \n";
			if (m_node->GetNodeType() > 0){ // switch
				packet->AddPacketTag(FlowIdTag(m_ifIndex));
				m_node->SwitchReceiveFromDevice(this, packet, ch);
			}else { // NIC
				// send to RdmaHw
				std::cout<<Simulator::Now()<<" receive normal packet at normal node rdma hw \n";
		...
		QbbNetDevice::TransmitStart(Ptr<Packet> p)
	{
		...
		std::cout<<Simulator::Now()<<" m_bps is "<<m_bps<<"\n";
		Time txTime = Seconds(m_bps.CalculateTxTime(p->GetSize()));
		std::cout<<Simulator::Now()<<" txtime for packet in sec is "<<txTime.GetSeconds()<<"\n";
		Time txCompleteTime = txTime + m_tInterframeGap;
		std::cout<<Simulator::Now()<<" txCompleteTime for packet in sec is "<<txCompleteTime.GetSeconds()<<"\n";
		...
		if (result == false)
		{
			std::cout<<Simulator::Now()<<" result is false drop \n";
			m_phyTxDropTrace(p);
		}
		return result;
	}
   ...
   void QbbNetDevice::TriggerTransmit(void){
	...
	void QbbNetDevice::UpdateNextAvail(Time t){
		if (!m_nextSend.IsExpired() && t < m_nextSend.GetTs()){
			...
			std::cout<<"Simulator now "<<Simulator::Now()<<" scheduler later to "<<Simulator::Now()+delta<<" dequeue and transmit in UpdateNextAvail\n";
